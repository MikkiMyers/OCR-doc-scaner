...
type Stage = 'idle'|'preprocess'|'ocr'|'ai'|'done'|'error';
type Progress = { stage: Stage; progress: number; message?: string };
...
export default function Page() {
  ...
  const [status, setStatus] = useState<Progress>({ stage: 'idle', progress: 0 });
  const [cvReady, setCvReady] = useState(false);
  const [dragOver, setDragOver] = useState(false);
  const [useAI, setUseAI] = useState(true);          // << เปิด AI refine
  ...
  useEffect(() => {
    ...
    const check = () => {
      ...
      if (ok) {
        ...
        setCvReady(true); clearInterval(interval);
      }
    };
  ...
  const tidyText = useCallback((s: string) => {
    ...
    for (const l of lines) {
      const isBullet = /^([•·●▪■\-–*]|\d+\)|\(\d+\))\s+/.test(l);
      const isHeading = /(ประสบการณ์การทำงาน|ประสบการณ์ทำงาน|ติดต่อ|เกี่ยวกับ|ประวัติการศึกษา|ทักษะ|ทักษะทางภาษา|รางวัลที่ได้รับ|INVOICE|RECEIPT|BILL|Summary|Profile|Education|Skills|Languages|Awards)/i.test(l);
  ...
  // ===== AI refine (ใหม่)
  const aiRefine = useCallback(async (raw: string) => {
    setStatus({ stage: 'ai', progress: 0.05, message: 'กำลังปรับภาษา/จัดหัวข้อด้วย AI...' });
    const res = await fetch('/api/ai-refine', {
      method: 'POST', headers: { 'Content-Type':'application/json' },
      body: JSON.stringify({ text: raw })
    });
    const json = await res.json();
    if (!res.ok) throw new Error(json.error || 'AI refine error');
    return json as { cleanText: string; sections: any[]; fields: SmartFields };
  }, []);
  ...
  const runPipeline = useCallback(async () => {
    ...
    try {
      ...
      for (let i = 0; i < contours.size(); i++) {
        ...
        if (approx.rows === 4) { const area = cv.contourArea(approx); if (area > maxArea) { maxArea = area; if (approxCandidate) approxCandidate.delete(); approxCandidate = approx; } else approx.delete(); }
        else approx.delete();
      }
      ...
      // ---- tidy + parse
      let cleaned = tidyText(out);
      setText(cleaned);

      const inv = parseInvoice(cleaned);
      const langForSmart: 'auto'|'tha'|'eng' = ocrLang === 'eng' ? 'eng' : ocrLang === 'tha' ? 'tha' : 'auto';
      let sp = smartParse(cleaned, { lang: langForSmart });

      // ถ้าเปิด AI หรือเป็น resume เพี้ยนหนัก → ใช้ AI refine
      if (useAI) {
        try {
          const ai = await aiRefine(cleaned);
          if (ai?.cleanText) cleaned = ai.cleanText;
          // merge sections/fields (AI เป็นหลัก ถ้าไม่มีค่อยใช้ sp)
          sp = {
            sections: (ai?.sections?.length ? ai.sections : sp.sections) as any,
            fields: { ...sp.fields, ...(ai?.fields||{}) },
            lineItems: [],
          };
          setText(cleaned);
        } catch (e:any) {
          setToast({ type:'error', message: `AI refine ล้มเหลว: ${e?.message||e}` });
        }
      }

      // ถ้าเป็นเอกสารธุรกิจ → ใช้ invoice parser ครอบ
      ...
      setStatus({ stage: 'error', progress: 0, message: err?.message || String(err) });
      setToast({ type:'error', message: `ผิดพลาด: ${err?.message||err}` });
      setTimeout(()=>setToast(null), 3200);
    }
  }, [imageURL, cvReady, ocrLang, getWorker, cloudOCR, urlToDataURL, tidyText, useAI, aiRefine]);
    ...
    <main className="container">
      ...
      <header className="app-header">
        <div className="inner">
          ...
          <div className="small muted">ไทย/อังกฤษ · OpenCV + Tesseract · Smart Parser + AI</div>
          <div className="grow" />
          <button className="btn btn-outline btn-sm" onClick={downloadJSON} disabled={!text}><Download size={16}/> ดาวน์โหลด JSON</button>
        </div>
      </header>
      ...
      >
        <div className="stack">
          <div className="flex flex-center"><div className="badge"><ScanLine size={16}/> อัปโหลดเอกสาร</div></div>
        ...
        <div className="stack">
          <div className="card">
            ...
            <div className="card-body">
              <div className="media-box">{imageURL ? <img src={imageURL} alt="original" /> : <div className="small muted p-4">ยังไม่เลือกรูป</div>}</div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="card-body">
              <div className="media-box"><canvas ref={canvasRef} style={{ width:'100%' }} /></div>
              ...
              {status.stage !== 'idle' && (
                <div className={`status mt-3 status--${status.stage}`}>
                  ...
                  <span className="label">
                    {status.stage==='preprocess' && 'เตรียมภาพ'}
                    {status.stage==='ocr'        && 'อ่านตัวอักษร'}
                    {status.stage==='ai'         && 'ปรับภาษา/จัดหัวข้อ (AI)'}
                    {status.stage==='done'       && 'เสร็จสมบูรณ์'}
                    {status.stage==='error'      && 'ผิดพลาด'}
                  </span>
          ...
          <div className="card">
            <div className="card-header">
              ...
              <div className="flex flex-center">
                ...
                <label className="flex flex-center small" style={{gap:8, marginLeft:12}}>
                  <input type="checkbox" checked={useAI} onChange={(e)=>setUseAI(e.target.checked)} />
                  ใช้ AI ปรับภาษา/จัดหัวข้อ <Wand2 size={16}/>
                </label>
                <button
                  onClick={async () => {
                    if (!text) return;
                    try {
                      ...
                      setText(ai.cleanText || text);
                      setSections(ai.sections?.length ? ai.sections : sections);
                      setDocFields({ ...(docFields||{docType:'generic'}), ...(ai.fields||{}) });
                      setToast({ type:'success', message:'ปรับภาษา/จัดหัวข้อด้วย AI แล้ว' });
            ...
            <div className="card-body">
                  ...
                  <div className="stack">
                    ...
                    {docFields.docType === 'resume' ? (
                      <>
                        ...
                        {(docFields.phone || docFields.email || docFields.address) && (
                          <>
                            {docFields.phone && <div className="small"><b>โทร:</b> {docFields.phone}</div>}
                            {docFields.email && <div className="small"><b>อีเมล:</b> {docFields.email}</div>}
                            {docFields.address && <div className="small"><b>ที่อยู่:</b> {docFields.address}</div>}
                          </>
                        )}
                      </>
                    ) : (
              ...
              {/* line items (กรณีเป็น invoice) */}
              {lineItems?.length ? (
                <div className="mt-4">
                  <table className="table">
                    <thead><tr><th>รายการ</th><th className="num">จำนวน</th><th className="num">ราคาต่อหน่วย</th><th className="num">เป็นเงิน</th></tr></thead>
                    <tbody>
                      {lineItems.map((it,i)=>(
                        <tr key={i}><td>{it.description}</td><td className="num">{it.qty??''}</td><td className="num">{it.unitPrice??''}</td><td className="num">{it.amount??''}</td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
              ...
              <div className="grid-2 mt-6">
                <div>
                  <h3 className="small muted">ข้อความดิบ (หลังขัด)</h3>
                  <div className="json-viewer mt-2"><pre><code>{text || '—'}</code></pre></div>
                </div>
                <div>
                  <h3 className="small muted">แยกเป็นหัวข้อ</h3>
                        ...
                        <div className="card-body">
                          {s.content?.length ? (
                            <ul className="stack" style={{paddingLeft:'1rem', listStyle:'disc'}}>
                              {s.content.map((c: string, ci: number)=>(<li key={ci} className="small">{c}</li>))}
                            </ul>
            ...
            <div className="card-footer small muted">ใช้ OpenCV.js + Tesseract.js + (ตัวเลือก) AI Refine — ประมวลผลบนเบราว์เซอร์/Serverless</div>
          </div>
        </div>
      </section>
      ...
      {toast && (
        <div className={`toast ${toast.type==='success'?'success':'error'}`}>
          <strong>{toast.type==='success' ? 'สำเร็จ' : 'ผิดพลาด'}</strong>
